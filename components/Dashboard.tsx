...
const Dashboard: React.FC<Props> = ({ profile, onLogout }) => {
  const [activeTab, setActiveTab] = useState<TabType>(() => (localStorage.getItem(`rav_active_tab_${profile.id}`) as TabType) || 'LAUNCHER');
  
  // Intelligence Caches
  const [briefing, setBriefing] = useState<string | null>(() => localStorage.getItem(`rav_briefing_${profile.id}`));
  const [directive, setDirective] = useState<string | null>(() => localStorage.getItem(`rav_directive_${profile.id}`));
  const [shadowDirective, setShadowDirective] = useState<string | null>(() => localStorage.getItem(`rav_shadow_${profile.id}`));
  const [portrait, setPortrait] = useState<string | null>(() => localStorage.getItem(`rav_portrait_${profile.id}`));
  const [assetVideo, setAssetVideo] = useState<string | null>(null); // Video BLOBs don't persist well in LS, so we regenerate or just flag it
  const [chatHistory, setChatHistory] = useState<ChatMessage[]>([]);
  const [intelResult, setIntelResult] = useState<string | null>(() => localStorage.getItem(`rav_last_intel_${profile.id}`));
  const [yearlyData, setYearlyData] = useState<{ month: string, freq: number, directive: string }[]>(() => {
    const saved = localStorage.getItem(`rav_yearly_${profile.id}`);
    return saved ? JSON.parse(saved) : [];
  });
  const [missions, setMissions] = useState<TacticalMission[]>(() => {
    const saved = localStorage.getItem(`rav_missions_${profile.id}`);
    return saved ? JSON.parse(saved) : [];
  });
  ...
  const [compTargetName, setCompTargetName] = useState(() => localStorage.getItem(`rav_form_comp_name_${profile.id}`) || '');
  const [compTargetDob, setCompTargetDob] = useState(() => localStorage.getItem(`rav_form_comp_dob_${profile.id}`) || '');
  const [compResult, setCompResult] = useState<{ score: number, summary: string } | null>(null);
  const [loadingComp, setLoadingComp] = useState(false);
  const [exploreSearchInput, setExploreSearchInput] = useState(() => localStorage.getItem(`rav_form_explore_query_${profile.id}`) || '');
  const [reportInput, setReportInput] = useState(() => localStorage.getItem(`rav_form_report_text_${profile.id}`) || '');
  ...
  const [dailyIntelCache, setDailyIntelCache] = useState<Record<string, string>>(() => {
    const saved = localStorage.getItem(`rav_daily_cache_${profile.id}`);
    return saved ? JSON.parse(saved) : {};
  });
  ...
  // Persistence Effects
  useEffect(() => { localStorage.setItem(`rav_active_tab_${profile.id}`, activeTab); }, [activeTab, profile.id]);
  useEffect(() => { localStorage.setItem(`rav_allies_${profile.id}`, JSON.stringify(allies)); }, [allies, profile.id]);
  useEffect(() => { localStorage.setItem(`rav_reports_${profile.id}`, JSON.stringify(fieldReports)); }, [fieldReports, profile.id]);
  useEffect(() => { localStorage.setItem(`rav_yearly_${profile.id}`, JSON.stringify(yearlyData)); }, [yearlyData, profile.id]);
  useEffect(() => { localStorage.setItem(`rav_missions_${profile.id}`, JSON.stringify(missions)); }, [missions, profile.id]);
  useEffect(() => { localStorage.setItem(`rav_daily_cache_${profile.id}`, JSON.stringify(dailyIntelCache)); }, [dailyIntelCache, profile.id]);
  
  // Input Persistence Effects
  useEffect(() => { localStorage.setItem(`rav_form_comp_name_${profile.id}`, compTargetName); }, [compTargetName, profile.id]);
  useEffect(() => { localStorage.setItem(`rav_form_comp_dob_${profile.id}`, compTargetDob); }, [compTargetDob, profile.id]);
  useEffect(() => { localStorage.setItem(`rav_form_explore_query_${profile.id}`, exploreSearchInput); }, [exploreSearchInput, profile.id]);
  useEffect(() => { localStorage.setItem(`rav_form_report_text_${profile.id}`, reportInput); }, [reportInput, profile.id]);
  ...
  const handleLiveUplink = async () => {
    ...
    try {
      const callbacks = {
        ...
        onerror: (e: any) => { console.error("Live Error", e); setIsLiveActive(false); },
  ...
  const fetchAssetVideo = async () => {
    ...
    if (result) setAssetVideo(result);
  ...
  const fetchDirective = async () => {
    ...
    setDirective(result);
    localStorage.setItem(`rav_directive_${profile.id}`, result);
  ...
  const fetchShadowDirective = async () => {
    ...
    localStorage.setItem(`rav_shadow_${profile.id}`, result);
  ...
  const searchIntel = async (query: string) => {
    ...
    if (query.includes(',') || query.length > 10) {
      ...
      localStorage.setItem(`rav_last_intel_${profile.id}`, mapsIntel);
    } else {
      const result = await fetchFoundingIntel(query);
      setIntelResult(result);
      localStorage.setItem(`rav_last_intel_${profile.id}`, result);
    }
  ...
  const handleDateSelect = async (date: string) => {
    ...
    if (dailyIntelCache[date]) return;
    setLoadingDaily(true);
    playTacticalSound('click');
    const result = await fetchDailyFrequency(date, profile);
    setDailyIntelCache(prev => ({ ...prev, [date]: result }));
  ...
  const SettingsView = () => (
    <div className="max-w-4xl mx-auto py-10 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
             ...
             <TacticalButton variant="danger" className="w-full" icon={<Trash2 size={14}/>} onClick={() => {
               if (confirm("CONFIRM ARCHIVE PURGE? This action cannot be reversed.")) {
                 Object.keys(localStorage).forEach(key => {
                   if (key.includes(profile.id) || key.startsWith('rav_')) {
        ...
        <GlassCard title="SESSION INTEGRITY">
           <div className="py-4 space-y-6 flex flex-col items-center">
              ...
              <div className="text-center">
                ...
                <p className="text-[8px] font-mono text-gray-600 uppercase">Archive Auto-Persistence Active</p>
              </div>
              <div className="flex gap-2 items-center text-[9px] font-mono text-green-500/60 uppercase">
                <Save size={12} /> Data Synchronized
              </div>
           </div>
        </GlassCard>
      </div>
                     ...
                     <div className="p-8 bg-black/40 rounded-2xl border border-white/5 min-h-[160px] flex items-center justify-center">
                        {loadingDaily ? <Loader2 className="animate-spin text-agency-gold" /> : (
                          <p className="text-sm font-mono text-center max-w-lg leading-relaxed uppercase tracking-tight">
                            {dailyIntelCache[selectedDate] || "Select a sequence node to decode frequency pulse."}
                          </p>
                        )}
                     </div>
                   </GlassCard>
